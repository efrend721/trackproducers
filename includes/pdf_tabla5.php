...
foreach ($resultsMachine as $resultsMachines) {
    ...
    $stmtCarts = $dbh->connect()->prepare($consultas['details']['queryCommentsAllRangeDate']);
    $stmtCarts->bindParam(1, $area);
    $stmtCarts->bindParam(2, $startDate);
    $stmtCarts->bindParam(3, $endDate);
    $stmtCarts->execute();
...
$baseFileName = 'dtr_all_range_date_machines';
